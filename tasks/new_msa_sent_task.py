...
from jobDiva.new_updated_contact_note_records import fetch_contact_note_records as get_new_msa_records
from settings import team_map
from utils import count_msa, populate_objectives_data, get_last_week_range, get_obj_data_range
...
class CreateNewMSAsSent(Task):
    def run(self):
        try:
            print("\n\nCreating new MSA records ...")
            team = "SALES_TEAM"
            job_diva_auth_token = authenticate_jobdiva()
            #  Get all new job records

            response_data = get_new_msa_records(job_diva_auth_token)
            for member in team_map[team]:
                ...
                meetings_conducted = count_msa(response_data, jd_user_id)

                print(f"Total MSA sent by {member}: {meetings_conducted}")
                ...
                objectives = populate_objectives_data(
                    ...
                    objective_description=f"New MSAs Sent - {date_range}",
                    start_ts=start_ts,
                    end_ts=end_ts,
            ...
            logger.error(f"Error in NewMSAsSent task: {str(e)}")


create_new_msa_task = app.register_task(CreateNewMSAsSent())
